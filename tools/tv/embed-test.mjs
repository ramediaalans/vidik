...
async function vibixFrame() {
  ...
  const d = j.data ?? j;
  return { url: d.iframe_url, note: `${d.name} (${d.year}), ${d.duration} мин` };
}
...
<div class="row">
  <div><h2>vibix — ${vibix.note}</h2><iframe id="vibix" src="${vibix.url}" allow="autoplay; fullscreen" allowfullscreen></iframe></div>
...
const who = (source) => {
  for (const id of ['vibix','rutube','vk']) {
...
setTimeout(() => { report.done = true; say('--- готово ---'); }, 22000);
<\/script>
</body></html>`;
