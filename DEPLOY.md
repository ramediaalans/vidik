...
2. **Вынести медиа на отдельный хост** через `VITE_MEDIA_BASE` — см. ниже.

## VITE_MEDIA_BASE: медиа на отдельном хосте

`app/src/media/asset.ts` отправляет на внешний хост только три префикса:
`music/`, `roms/`, `cores/`. Картинки и постеры остаются рядом с сайтом.

```
VITE_MEDIA_BASE=https://media.домен      # без слеша в конце, впрочем он отрезается
```

Пусто или не задано — всё берётся из `app/public/`, как раньше (режим разработки).

Структура на внешнем хосте повторяет `public/`: `media.домен/music/…`, `/roms/…`, `/cores/…`.

Что важно на стороне хоста:

- **CORS.** Всё три папки читаются через `fetch`/XHR (Webamp декодирует звук для
  визуализации, Nostalgist тянет ром и wasm), так что без `Access-Control-Allow-Origin`
  музыка и эмулятор не заведутся.
- **Content-Type для `.wasm`** должен быть `application/wasm`, иначе стримовая
  компиляция ядра отвалится.
- Файлы неизменяемые, можно ставить долгий `Cache-Control`.
